...
import { getTechnologyBySlug } from "@/services/taxonomy";
...
export default async function TechnologyPage({
  ...
  const basePath = `/technologies/${slug}`;

  return (
    <main className="mx-auto max-w-6xl px-6 py-12">
      <h1 className="mb-6 font-display text-3xl font-bold">{technology.name}</h1>

      <SortLinks basePath={basePath} currentParams={sp} activeSort={filters.sort} />

      {projects.length === 0 ? (
        <p className="text-sm text-muted-foreground">No projects use this yet.</p>
      ) : (
